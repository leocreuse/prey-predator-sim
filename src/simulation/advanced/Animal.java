...
public class Animal extends Circle {
	...
	private double viewing_distance;
	
	private double viewing_angle;
	...
	public Animal(double centerX, double centerY, Color col, double hungerRate, double reproProb, double age,
			double hunger, double speed, double repRad,double viewing_distance, double viewing_angle, Cst cst) {
		...
		this.viewing_distance = viewing_distance;
		this.viewing_angle = viewing_angle;
	}

	/** baby constructor */
	public Animal(Animal original,double random) {
		...
		this.dep.setMovement(this.dep.getSpeed(), 2*random*Math.PI);
		this.reproductionRadius = original.reproductionRadius;
		this.viewing_distance = original.viewing_distance;
		this.viewing_angle = original.viewing_angle;
	}
	...
	public static boolean canReproduce(Animal a1, Animal a2) {
		...
		return !a2.hasReproduced && !a1.hasReproduced && a1.age > 4 && !a1.hasDied && a2.age > 4 && !a2.hasDied
	...
	/**
	 * default next movement is generated with the same speed and
	 * a random direction, if random is a double randomly generated between 0 and 1
	 */
	public void computeNextMovement(double random) {
		this.setMovement(this.dep.getSpeed(), 2 * Math.PI * random);
	}

	/** checks if this is within the borders of the simulation or not*/
	 ...
	 * distance is used to find the closes living animal within line of sight.
	 */
	public static double deadComp(Animal orig, Animal other) {
		double res = 0;
		res = other.hasDied ? 2 * Simulation.SIZE : 0;
		res+=(distance(orig,other)>orig.viewing_distance || Math.abs(Math.atan2(other.getCenterY()-orig.getCenterY(),other.getCenterY()-orig.getCenterY()) - orig.getDep().getDirection())>orig.viewing_angle/2)?2 * Simulation.SIZE : 0;
	...
	/**
	 * this function returns the "reproduction distance" between orig and
